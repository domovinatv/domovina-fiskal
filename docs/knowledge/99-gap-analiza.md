...
| R3 | XML-DSIG: Exclusive C14N + RSA-SHA1 + SHA1 digest; CIS **odgovor** koristi inkluzivni C14N | 02 | Spec. v2.6 §7; test verifikacije odgovora | ❌ **DJELOMIČNO OBORENO (2026-07-05, faza 2, CIS TEST):** exc-C14N ✅, ali **RSA-SHA1/SHA1 vraća `s004`** — CIS danas traži **RSA-SHA256 + SHA-256 digest** (JIR dobiven). ZKI ostaje RSA-SHA1+MD5. Ispravljeno u `02` §6.1/§12. |
...
| R16 | Prag ulaska u sustav PDV-a 100.000 €; tekst oslobođenja malog obveznika **čl. 90. st. 1.** | 06, 10 | Zakon o PDV-u | ✅ **RAZRIJEŠENO (2026-07-04, faza 1):** od **1. 1. 2025.** klauzula glasi *„Oslobođeno plaćanja PDV-a prema **čl. 90. st. 1.** Zakona o porezu na dodanu vrijednost."* (do 31. 12. 2024. bio je st. 2. — otud nekonzistentnost u `01`/`06`). Potvrđeno na [TEB](https://www.teb.hr/novosti/2025/klauzula-o-oslobodenju-od-placanja-pdv-a-na-racunima-malih-poreznih-obveznika-od-112025/); implementirano u `backend/src/validacija.ts` (`KLAUZULA_CL90`). |
...
4. **B2C fiskalizacija 1.0** — ✅ **IMPLEMENTIRANO (faza 2, 2026-07-05).** ZKI (RSASSA-PKCS1-v1_5+SHA-1 → MD5, na Workeru) + XML-DSIG (exc-C14N/**RSA-SHA256/SHA-256** — v. R3!) + SOAP `RacunZahtjev` na CIS TEST `:8449` s DEMO certom; JIR dobiven. **Razriješeno: transportni mTLS NIJE obavezan → sidecar NE treba**; ali CIS-ov poslužiteljski cert je Fina (privatni CA) pa transport ide `cloudflare:sockets` + `subtls` (v. `02` §12). Async + naknadna dostava (`NakDost=true`, novi `IdPoruke`, rok **2 radna dana**) kroz cron sweep.
