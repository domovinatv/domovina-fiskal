...
- ⚠️ **1. XAdES potpis u MPS specifikaciji (SignedServiceMetadata) — rsa-sha1 vs rsa-sha256.** Djelomično razriješeno: pronađena **kontradikcija unutar samog MPS dokumenta** (doc 141, v1.3, 8.10.2025.). **Normativni tekst §4.2.1 "Sigurnost"** eksplicitno propisuje: *"SignatureMethod mora biti: `http://www.w3.org/2000/09/xmldsig#rsa-sha1`"* i *"DigestMethod mora biti: `http://www.w3.org/2000/09/xmldsig#sha1`"* (kanonikalizacija `http://www.w3.org/TR/2001/REC-xml-c14n-20010315`). **ALI priloženi XML primjer `SignedServiceMetadata`** u istom dokumentu koristi `SignatureMethod ...xmldsig-more#rsa-sha256` + `DigestMethod ...xmlenc#sha256`. Dakle prozni zahtjev (naslijeđen iz OASIS BDXR/Peppol SMP predloška) i radni primjer se **ne slažu**. Koje je mjerodavno nije javno pojašnjeno — treba potvrditi na PTS-u / referentnom DomiSMP-u (DomiSMP defaultno potpisuje **SHA-256**). Preporuka za implementaciju: pratiti **primjer (rsa-sha256/sha256)**, ali pripremiti fallback na sha1 dok se ne potvrdi na PTS-u. Izvor: [MPS doc 141](https://porezna.gov.hr/fiskalizacija/api/dokumenti/141) (PDF povučen 2026-07-04; API endpoint povremeno 500, PDF stream čitljiv), mirror [fiskalizacija2.hr — Tehnicka_specifikacija_eRacun_MPS.pdf](https://fiskalizacija2.hr/wp-content/uploads/2025/07/Tehnicka_specifikacija_eRacun_MPS.pdf). NB: fiskalne poruke (`evidentiraj*`) su ODVOJENO potvrđene na **rsa-sha256** (v. točka 2).

- ✅ **2. Nazivi elemenata fiskalne poruke `evidentiraj*`, KPD listID, jedinice mjere.** Razriješeno iz **"Tehnička specifikacija Fiskalizacija eRačuna i eIzvještavanje" v1.3 (20.6.2025.)** (94 str.), mirror [fiskalizacija2.hr PDF](https://fiskalizacija2.hr/wp-content/uploads/2025/07/Tehnicka_specifikacija_Fiskalizacija_eRacuna_i_eIzvjestavanje.pdf). Fiskalne/izvještajne poruke (SOAP, XAdES-B, enveloped): **`EvidentirajERacun`**, **`EvidentirajIsporukuZaKojuNijeIzdanERacun`**, **`EvidentirajInozemniERacun`** (novo — inozemni eRačun), **`EvidentirajNaplatu`**, **`EvidentirajOdbijanje`** — svaka s parom `...Zahtjev`/`...Odgovor`. Namespace: `http://www.poreznauprava.gov.hr/fin/2024/types/eFiskalizacija` (fiskalizacija) i `.../eIzvjestavanje`. **KPD klasifikacija**: `cbc:ItemClassificationCode/@listID` = **`'CG'`** (+ `@listVersionID`) za KPD nomenklaturu. **Jedinice mjere**: šifrarnik **EN16931 (UN/ECE Rec 20)**, upisuje se u `cbc:InvoicedQuantity/@unitCode` (i `CreditedQuantity/@unitCode`); popis u dodatku spec-a "Šifrarnik jedinica mjera po EN16931". **CIUS identifikatori** ugrađeni: `urn:mfin.gov.hr:cius-2025:1.0` (compliant) + `urn:mfin.gov.hr:ext-2025:1.0` (conformant), UBL 2.1. NAPOMENA: zasebni CIUS dokument (api/dokumenti/**99**) i dalje vraća **HTTP 500** preko API-ja (nedostupan javno), ali su relevantni elementi/identifikatori pokriveni gornjom fiskalizacijskom spec. i MPS dokumentom.

- ✅ **3. Produkcijski CIS endpoint + WSDL za `evidentiraj*` (F2.0), odvojeno od B2C `:8449`.** Razriješeno (ista spec. v1.3, tablica 103): **Produkcija = `https://cis.porezna-uprava.hr:8509/FiskalizacijaService`**, **Test = `https://cistest.apis-it.hr:8509/FiskalizacijaServiceEprod`**. Port **8509** (razlikuje se od B2C fiskalizacije 1.0 na `:8449`). Poslužiteljski certifikati: `cis.porezna-uprava.hr` (prod) / `cistest.porezna-uprava.hr` (test); aplikativni: `fiskalcis` / `fiskalcistest`. WSDL servisi: `eFiskalizacijaService` i `eIzvjestavanjeService` (namespace `http://www.poreznauprava.gov.hr/fin/2024/services/...`). Potpis poruke: **XAdES-B, XML enveloped, `rsa-sha256` + `sha256`** (potvrđeno primjerom `EvidentirajERacunOdgovor`). Izvor: fiskalizacija2.hr PDF (gore), §10–11 + tablica 103.

- ✅ **4. NIS2 obveze informacijskog posrednika kao "ključnog subjekta".** Razriješeno: **Zakon o kibernetičkoj sigurnosti, NN 14/2024**, na snazi od **15.2.2024.** (hrvatska transpozicija NIS2). Obveze ključnog subjekta: (a) **samoidentifikacija/registracija** kod nadležnog tijela (rok za popis obveznika bio je 2025.; prijava se radi kod nadležnog sektorskog tijela / središnjeg tijela za kibernetičku sigurnost, incidenti idu na **CERT.hr / nadležni CSIRT**); (b) **prijava incidenata u tri faze**: **rano upozorenje ≤ 24 sata**, **obavijest o incidentu ≤ 72 sata**, **konačno izvješće ≤ 1 mjesec**; (c) mjere upravljanja rizicima + nadzor; kazne **do 10 mil. EUR ili 2 % prometa**. Izvori: [zakon.hr — Zakon o kibernetičkoj sigurnosti](https://www.zakon.hr/z/3718/zakon-o-kibernetickoj-sigurnosti), [NCSC-HR](https://ncsc.hr/hr/zakon-o-kibernetickoj-sigurnosti-182), [visioncompliance.eu — NIS2 vodič HR](https://visioncompliance.eu/hr/blog/nis2-direktiva-vodic-hrvatska). (Točni članci ZKS-a za rokove prijave preporuka je citirati iz NN 14/2024 pri finalizaciji compliance dokumentacije.)

- ⚠️ **5. Službeni PU zahtjev za minimalni SLA/dostupnost PT-a (24/7) i retencijske rokove poruka.** Djelomično: **PU ne objavljuje formalni SLA/postotak dostupnosti (npr. 24/7 uptime)** za pristupne točke u javnim tehničkim dokumentima (doc 140 v1.4) — nema izdvojenog SLA dokumenta ni brojčane raspoloživosti. Jedine kvantificirane vrijednosti su **P-mode/Domibus parametri** iz priloga doc 140: **`retention_undownloaded="14400"`**, **`retention_sent="14400"`** (sekundi = **4 sata** zadržavanja u redu poruka), `retention_downloaded="0"`, `retention_metadata_offset="0"`, te **`maxretries=10, period=3000` (ms)** za ponovno slanje. To su **operativni parametri reda poruka AS4 gatewaya**, NE zakonski arhivski rok (arhiviranje eRačuna kao knjigovodstvene isprave uređeno je zasebno računovodstvenim/PDV propisima ~11 god., izvan opsega PT SLA). Zaključak: **javno ne postoji propisan minimalni SLA/dostupnost PT-a**; postoji samo default P-mode retencija (14400 s) i retry politika. Izvor: [Pristupna točka i AS4 profil, doc 140, v1.4](https://porezna.gov.hr/fiskalizacija/api/dokumenti/140) (Prilog 4 — Domibus P-mode; PDF povučen 2026-07-04).
